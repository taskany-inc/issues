...
export const HistoryRecordEstimate: React.FC<HistoryChangeProps<string>> = ({ from, to }) => {
    ...
    const fromType = fromEstimate?.type === 'Year' ? 'Year' : 'Quarter';
    const toType = toEstimate?.type === 'Year' ? 'Year' : 'Quarter';

    return (
        <HistorySimplifyRecord
            from={
                fromEstimate ? (
                    <Tag size="s">{formateEstimate(fromEstimate.date, { locale, type: fromType })}</Tag>
                ) : null
            }
            to={toEstimate ? <Tag size="s">{formateEstimate(toEstimate.date, { locale, type: toType })}</Tag> : null}
        />
    );
};
