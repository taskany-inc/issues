...
export const createQuarterRange = (q: QuartersKeys, year?: number): DateRange => {
    ...
    const qEndDate = (date: number) => {
        ...
        const start = createDateAsUTC(new Date(d.getFullYear(), quarter * 3, 1));

        return {
            end: createDateAsUTC(new Date(start.getFullYear(), start.getMonth() + 3, 0)),
            start,
        };
    };
...
const createYearRange = (year: number): DateRange => {
    ...
    return {
        start: createDateAsUTC(new Date(date.getFullYear(), 0, 1)),
        end: createDateAsUTC(new Date(date.getFullYear(), 11, 31)),
    };
};
...
export const urlDateFormat = (date: Date) => date.toISOString().split('T')[0];
