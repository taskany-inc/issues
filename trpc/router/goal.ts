...
import { addGoalMetaDataToJiraIssue, jiraService, JiraUser, searchIssue } from '../../src/utils/integration/jira';
...
export const goal = router({
    ...
    recognizeGoalScopeIdById: protectedProcedure.input(z.string()).query(async ({ input }) =>
        prisma.goal
            ...
            // if error we try another search way
            .catch(() => {
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            if (input.criteriaGoal?.id) {
                ...
                if (externalTask) {
                    ...
                    await addGoalMetaDataToJiraIssue({
                        jiraKey: externalTask.externalKey,
